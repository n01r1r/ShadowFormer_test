...
class DataLoaderVal(Dataset):
    def __init__(self, rgb_dir, target_transform=None):
        ...
        input_dir = ''  # Use only shadowed images as input
        self.image_files = sorted(os.listdir(os.path.join(rgb_dir, input_dir)))

        self.image_filenames = [os.path.join(rgb_dir, input_dir, x) for x in self.image_files if is_png_file(x)]

        self.tar_size = len(self.image_filenames)
    ...
    def __getitem__(self, index):
        ...
        shadowed_img = torch.from_numpy(np.float32(load_img(self.image_filenames[tar_index])))
        shadowed_img = shadowed_img.permute(2, 0, 1)  # Change to CHW format for PyTorch
        
        filename = os.path.split(self.image_filenames[tar_index])[-1]

        return shadowed_img, filename  # Return only the shadowed image and filename
